...
int value2()
{
	...
	while (1)
	{
		for (int i = 0; i < 15; i++)//每一列
		{
			for (int j = 0; j < 15; j++)//每一行
			{
				if (map[i][j] == 2)//以一个棋子为中心发散判断，遇到空或其他颜色的棋子跳出
				{
					int colour = map[i][j];
					int m, n;
					int x;
						...
						while (1)
						{
							...
							else
							{
								...
								else if (map[m][n] == 0)
								{
									...
									if (flag == 0 && map[m][n] == colour && record == 2)
									{
										if (f == 1) { x_kill = m + 1; y_kill = n; }
										else if (f == 2) { x_kill = m; y_kill = n + 1; }
										else if (f == 3) { x_kill = m + 1; y_kill = n + 1; }
										else if (f == 4) { x_kill = m + 1; y_kill = n - 1; }
									}
									if (flag == 1 && record == 4)
									{
										if (f == 1) { x_kill = m + 1; y_kill = n; }
...
void local_reduction()//再次对第一层进行位置上的剪枝
{
	int an[15][15];
	for (int i = 0; i < 15; i++)
	{
	...
	for (int i = 0; i < 15; i++)
	{
		for (int j = 0; j < 15; j++)
		{
			if (flag[1][i][j] == 0 && an[i][j]== -1000000000 && map[i][j]==0)
			{
				map[i][j] = 1;
				an[i][j] = value();
				map[i][j] = 0;
			}
			else if (flag[1][i][j] != 0 || map[i][j] != 0)
			{
				an[i][j] = 0;
			}
		}
...
void traverse2()//先复制棋盘，再下棋进行遍历。遍历树，打算先深搜四层，先不写剪枝//可以不复制棋盘，只需要每一步评分之后删去棋子
{
...
void start1()//玩家选择先手
{
	...
	while (1)
	{
		...
		traverse();
		if (x_kill != -1 && y_kill != -1)
		{
			setfillcolor(WHITE);
			setcolor(WHITE);
			if (map[x_kill][y_kill] == 1)
			{
				fillcircle((x_now + 1) * 40, (y_now + 1) * 40, 20);
				map[x_now][y_now] = 1;
			}
			...
			{
				fillcircle((x_kill + 1) * 40, (y_kill + 1) * 40, 20);
				map[x_kill][y_kill] = 1;
				x_kill = -1; y_kill = -1;
			}
		}
